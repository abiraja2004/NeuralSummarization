...
if len(sys.argv)!=2:
    print('Usage: python fast_text_train.py <config>')
    exit(0)
...
if 'pretrain_embedding' in network_params and network_params['pretrain_embedding']==True:
    fasttext_model_params['embedding_matrix']=embedding_matrix
...
if 'model2load' in network_params and os.path.exists(network_params['model2load']):
...
for batch_idx in xrange(begin_batch_idx,batch_num+begin_batch_idx):
    input_matrix,masks,labels,_=my_data_manager.batch_gen(set_label='train',batch_size=my_network.batch_size,label_policy='min',model_tag='fasttext')
    ...
    if (batch_idx+1)%check_err_frequency==0:    # Error Checkpoint
    ...
    if (batch_idx+1)%validation_frequency==0:   # Validation Checkpoint
        my_network.dump_params(file2dump=model_saved_folder+os.sep+'%s_%d.ckpt'%(my_network.name,batch_idx+1))
        validation_loss=[]
        my_data_manager.init_batch_gen(set_label='validate',file_list=None,permutation=True)
        for validation_idx in xrange(validation_batches):
